...
public :
	...
	BOOL	Open(LPCWSTR filePathName, DWORD dwShareMode);	// 파일 직접 열기
	void	Close();
...
public :
	...
	void	Free() noexcept;
	void	Attach(BYTE* buf, INT64 size, BOOL bSelfFree=FALSE);

public :
	BYTE*	m_buf;			// 메모리 버퍼
	INT64	m_size;			// 버퍼 alloc 크기
	INT64	m_pos;			// 현재 읽는 위치
...
public :
	...
	void	Close(const FILETIME* mt = nullptr) noexcept;
	HANDLE	GetHandle() { return m_handle; }
	LPCWSTR GetFilePathName() { return m_filePathName; }
	BOOL	IsOpened() {return m_handle == INVALID_HANDLE_VALUE ? FALSE : TRUE;}
	//bool	SetFileTime(time_t t);
	//bool	SetFileTime(FILETIME* ct, FILETIME* at, FILETIME* mt);
...
public :
	BOOL	Alloc(INT64 size);			// 미리 쓸 크기를 아는 경우 사용한다.
	void	Free() noexcept;
	void	SetMaxAlloc(INT64 size) { m_maxAllocSize = size; }
	BYTE*	GetBuffer() { return m_buf; }
	void	Attach(BYTE* buf, INT64 len);
	INT64	GetAllocSize() { return m_allocSize; }
	void	SetSize(INT64 size) { m_size = size; }

private :
	BOOL	IncreaseBuffer(INT64 targetSize);
...
public :
	BYTE*	m_buf;			// 메모리 버퍼
	INT64	m_allocSize;	// 버퍼 alloc 크기
	INT64	m_size;			// 버퍼에서 사용중인 크기; 파일 크기에 해당된다.
	INT64	m_pos;			// 현재 위치 ; m_size 보다 커질 수 있다.
	BOOL	m_bSelfFree;	// 종료시 자동으로 메모리 해제할까 여부
	INT64	m_maxAllocSize;	// 최대 alloc 할 크기
};
...
////////////////////////////////////////////////////////////////////////////////////////////////////
///         dummy output stream - 출력 파일의 크기를 계산만 하고자 할 때 사용한다.
...
{
public :
	XDummyWriteStream() noexcept {m_size = 0; m_pos = 0;}
	...
	INT64	GetPos() final { return m_pos; }
...
{
public:
	XBuffer(const XBuffer& rhs);						// copy ctor
	const XBuffer& operator=(const XBuffer& rhs);		// copy operator
	XBuffer()
	{
		data = NULL;
	...
	BOOL	Alloc(int count)
	{
		...
		BYTE* temp = (BYTE*)realloc(data, count);
	...
	{
		if (data && allocSize == count) return TRUE;		// 동일하다. 할꺼 없다.
		if (count < dataSize) { ASSERT(0); return FALSE; }	// realloc 크기가 data 크기보다 작으면 안된다.
		BYTE* temp = (BYTE*)realloc(data, count);
		if (temp == NULL) return FALSE;
		data = temp;
		allocSize = count;
	...
	BOOL	CopyFrom(const XBuffer& rhs)
	{
		if (Alloc(rhs.allocSize) == FALSE) return FALSE;
		memcpy(data, rhs.data, rhs.dataSize);
		dataSize = rhs.dataSize;
		return TRUE;
	}
	BOOL	AllocAndCopyData(const BYTE* _data, int _size) 
	{
...
public :
	...
	~XFile(); 
	void		Close();
	bool		Open(LPCWSTR filePathName, DWORD access = GENERIC_READ, DWORD share = FILE_SHARE_READ, DWORD creation = OPEN_EXISTING, DWORD flags = FILE_FLAG_SEQUENTIAL_SCAN);
	bool		IsOpened() const {return (m_handle == INVALID_HANDLE_VALUE) ? false : true;}
	int			GetSize32() const;
	bool		ReadWhole(XBuffer& buffer) const;
	int			GetPos32() const;
	bool		CreateTempFileAndWriteData(LPCWSTR filePathName, const XBuffer& data);
	DWORD		GetLastError() { return m_err; }
