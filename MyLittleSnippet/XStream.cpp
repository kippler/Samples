...
INT64 XFileReadStream::SetPos(INT64 pos)
{
	...
	{
		int buf_passed = (int)(m_left - m_buf);	// 내부 버퍼에서 사용한 내역
	...
	LARGE_INTEGER li;
	li.QuadPart = pos;
	DWORD ret = ::SetFilePointer(m_handle, li.LowPart, &li.HighPart, FILE_BEGIN);	// 진짜로 파일 이동
...
INT64	XFileWriteStream::SetPos(INT64 pos)
{
	...
	{	
		DWORD err = ::GetLastError();	// 아마도 ERROR_NEGATIVE_SEEK 
		//ASSERT(0);
	}
	else
		m_virtPos = pos;
